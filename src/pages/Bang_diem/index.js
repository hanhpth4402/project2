...
function Bang_diem () {
    ...
    <main className="container">
            ...
            <div className="my-3 p-3 bg-body rounded shadow-sm">
                <h6 className="border-bottom pb-2 mb-0"><strong>Môn {userExam[0].TEN_MON}</strong></h6>
                    <div className="d-flex text-muted pt-3">
                        <svg className="bd-placeholder-img flex-shrink-0 me-2 rounded" width="32" height="32" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Placeholder: 32x32" preserveAspectRatio="xMidYMid slice" focusable="false"><title>Placeholder</title><rect width="100%" height="100%" fill="#007bff"/><text x="50%" y="50%" fill="#007bff" dy=".3em">32x32</text></svg>
                        <div className="pb-3 mb-0 small lh-sm border-bottom w-100">
                            <div className="d-flex justify-content-between">
                                <div>
                                    <strong className="text-gray-dark">{userExam[0].TEN_DE}</strong>
                                    <span className="d-block">{userExam[0].TYPE === 1 ? "Đề đầy đủ": "Đề rút gọn"}</span>
                                </div>
                                <div>
                                    <div> <strong>Điểm: {parseFloat(userExam[0].DIEM).toFixed(2)}</strong> </div>
                                    <div> <strong>Thời gian làm: {formatTime(parseInt(userExam[0].THOI_GIAN))}</strong> </div>
                                    <div> <strong>Ngày làm: {userExam[0].NGAY_THI.substring(0, 10)}</strong> </div>
                                </div>
